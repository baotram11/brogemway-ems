...
import { updateAccount, lockAccount, unlockAccount } from '../../store/slices/accountSlice';
import AccountModal from '../AccountModal/accountModal';

const AccountTable = (props) => {
	const { allAccounts, currentUser } = props;
	...
	const lockUser = (event, param) => {
		...
		return dispatch(lockAccount(currentUser.accessToken, update));
	};
	const unlockUser = (event, param) => {
		console.log('UNLOCKED');
		const update = { Account: param, Update: { IsActive: true } };
		return dispatch(unlockAccount(currentUser.accessToken, update));
	};
