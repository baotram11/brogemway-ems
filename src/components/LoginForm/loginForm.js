...
import { loginUser, selectCurrentUser, selectErrorLogin, selectLogin } from '../../store/slices/authSlice';
...
const LoginForm = () => {
	...
	useEffect(() => {
		if (currentUser != null) {
			const rawId = currentUser.account._id;
			const rawName = currentUser.account.Name;
			const rawToken = currentUser.accessToken;

			setAuthToken(rawToken);
			Cookies.set('token', rawToken);
			setUserId(rawId);
			Cookies.set('userId', rawId);
			setUserName(rawName);
			Cookies.set('userName', rawName);
		}
		if (login === 'succeeded') {
			return navigate(-1);
	...
	}, [setAuthToken, setUserId, setUserName, login, currentUser, setShow, navigate]);
